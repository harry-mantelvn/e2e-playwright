"""
AI-Powered Test Analysis System - GEMINI FREE VERSION
Zero-cost alternative using Google Gemini 1.5 Flash

Analyzes test results using:
1. Google Gemini 1.5 Flash for semantic analysis (FREE)
...
class TestAnalysisAI:
    """Main AI analysis orchestrator using Google Gemini (FREE)"""
    ...
    def __init__(self, gemini_api_key: Optional[str] = None):
        ...
        if self.api_key:
            try:
                ...
                print("✅ AI Analysis enabled with Google Gemini 1.5 Flash (FREE)")
            except Exception as e:
                print(f"⚠️  Failed to initialize Gemini: {e}")
                print("Running with statistical analysis only.")
                self.ai_enabled = False
        else:
            print("⚠️  GEMINI_API_KEY not found. Running with statistical analysis only.")
        ...
        self.analysis_results = {
            "analysis_metadata": {
                ...
                "model": "gemini-1.5-flash" if self.ai_enabled else "statistical-only",
                "provider": "Google Gemini Free" if self.ai_enabled else "None",
    ...
    def analyze(self, test_results: Dict, historical_data: Optional[Dict] = None) -> Dict:
        ...
        """
        print("🤖 Starting AI-powered test analysis with Gemini...")
        ...
        if failed_tests:
            ...
            # AI-powered failure categorization (Gemini)
            if self.ai_enabled:
    ...
    def _categorize_failures(self, failed_tests: List[Dict]) -> List[Dict]:
        ...
        for failure in failed_tests[:10]:  # Limit to avoid excessive API calls
            try:
    ...
    def _build_categorization_prompt(self, failure: Dict) -> str:
        """Build Gemini prompt for failure categorization"""
...
Be concise and specific."""
    ...
    def _analyze_root_causes(self, failed_tests: List[Dict]) -> List[Dict]:
        """Group failures and identify common root causes using Gemini"""
...
Be concise and actionable."""
    ...
    def _generate_summary(self, test_results: Dict) -> Dict:
        ...
        return {
            ...
            'ai_provider': 'Google Gemini Free' if self.ai_enabled else 'None',
            'cost': 'FREE'
        }
...
def print_summary(results: Dict):
    ...
    print("\n" + "="*60)
    print("🤖 AI ANALYSIS SUMMARY (Google Gemini FREE)")
    print("="*60)
    ...
    print(f"🤖 AI Provider: {summary.get('ai_provider', 'None')}")
    print(f"💰 Cost: {summary.get('cost', 'FREE')}")
...
def main():
    """Main execution"""
    print("🚀 AI-Powered Test Analysis (Google Gemini FREE Edition)")
    print("="*60)
    
    # Paths
    metrics_path = os.path.join('..', 'automationexercise-e2e-pom', 'test-summary', 'metrics.json')
    history_path = os.path.join('..', 'automationexercise-e2e-pom', 'test-summary', 'historical-data.json')
    output_path = os.path.join('..', 'automationexercise-e2e-pom', 'test-summary', 'ai-analysis.json')
    ...
    if not api_key:
        print("\n💡 TIP: Get your FREE Gemini API key at:")
        print("   https://aistudio.google.com/app/apikey")
        print()
    
    analyzer = TestAnalysisAI(gemini_api_key=api_key)
    ...
    print("💰 Cost: $0.00 (FREE with Google Gemini)")

...
npm run --silent | grep -E "clean|test|allure" || echo "⚠️  Some scripts might be missing"

# Test basic functionality
echo "✅ Running basic test validation..."
timeout 60 npx playwright test tests/basic.spec.ts --headed=false || echo "⚠️  Basic test validation failed (this is normal if no basic.spec.ts exists)"
